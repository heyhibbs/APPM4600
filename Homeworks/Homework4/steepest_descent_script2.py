...
def driver():
    ...
    #Gradient of q.
    def F(x):
        return np.array([x[0] + np.cos(x[0]*x[1]*x[2])-1, (1-x[0])**0.25+x[1]+0.05*x[2]**2-0.15*x[2]-1, -x[0]**2-0.1*x[1]**2+0.01*x[1]+x[2]-1 ]);

    def JF(x):
        return np.array([[2*x[0]-x[1]*x[2]*np.sin(x[0]*x[1]*x[2]),-x[0]*x[2]*np.sin(x[0]*x[1]*x[2]) , -x[0]*x[1]*np.sin(x[0]*x[1]*x[2])],[-0.25/((1-x[0])**.75) , 1 , 0.1*x[2]-0.15],[-2*x[0] ,0.01-0.2*x[1] , 1 ]]);
    ...
    for i in np.arange(nX):
        for j in np.arange(nY):
            for k in np.arange(nZ):
                V[i,j,k]=q(np.array([X[i,j],Y[i,j],Z[i,j]]));
